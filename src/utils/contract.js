...
const ADDRESS = "0x80e36b59E5ADDc58c245C5e3d69c81AEC4581b1D";
const NFT_ADDRESS = "0xB92A69717933714eCF60F544C0158A3743a3029c";
const TOKEN_ADDRESS = "0x57af5ED4756acd578CB11051c4C93cBf2Cb4BE1a";
		...
		"outputs": [
			{
				...
				"internalType": "struct MarketPlace.Path[]",
				"name": "_filtedPath",
				"type": "tuple[]"
			}
		],
  ...
  },
  {
    ...
    inputs: [
      ...
      },
      {
        ...
        indexed: false,
        internalType: "struct ERC721Tradable.Voting[]",
        name: "votings",
        type: "tuple[]",
      },
      {
...
const readContract = new ethers.Contract(ADDRESS, ABI, getProvider());
const readBUSD = new ethers.Contract(BUSD_ADDRESS, BUSD_ABI, getProvider());
let signer = getProvider().getSigner();
const writeContract = new ethers.Contract(ADDRESS, ABI, signer);
const writeNFT = new ethers.Contract(NFT_ADDRESS, NFT_ABI, signer);
const writeBUSD = new ethers.Contract(BUSD_ADDRESS, BUSD_ABI, signer);
...
const checkAllowance = async (userAddress) => {
  try {
    const tx = await readBUSD.allowance(userAddress, ADDRESS);
    return Number(tx);
  } catch (err) {
    console.log(err);
  }
...
const approveBUSD = async () => {
  const tx = await writeBUSD.approve(ADDRESS, LIMIT);
  const hash = tx.hash;
  return getProvider()
...
const buyNFT = async (nftId) => {
  const tx = await writeContract.buy(
    721,
    NFT_ADDRESS,
    nftId,
    "0xebf6e72f9D0ECc0B56e1755A3D3D9AFBb5B8BB92"
  );
  const hash = tx.hash;
  return getProvider()
    .waitForTransaction(hash)
...
const sendOffer = async (nftId, unit, price, startTime, duration) => {
  const tx = await writeContract.setOffers(
    721,
    NFT_ADDRESS,
    nftId,
    unit,
...
// _startTime: start time
// _duration: duration of auction
const updateOffer = async (nftAddress, nftId, offerPrice) => {
  const tx = await writeContract.updateOffers(nftAddress, nftId, offerPrice);
  const hash = tx.hash;
  return getProvider()
    .waitForTransaction(hash)
    .then(() => {
...
const acceptOffer = async (nftId, buyerAddress) => {
  const tx = await writeContract.accept(721, NFT_ADDRESS, nftId, buyerAddress);
  const hash = tx.hash;
  return getProvider()
    .waitForTransaction(hash)
    .then(() => {
...
const sellNFT = async (
  ...
  const tx = await writeContract.sell(
    721,
    NFT_ADDRESS,
    nftId,
    unit,
...
const updateOrder = async (nftId, openPrice) => {
  const tx = await writeContract.updateOrder(
    721,
    NFT_ADDRESS,
    nftId,
    openPrice
...
const setVoting = async (parentId = [], data = []) => {
  // Data: [
  //     startTime: Number,
  //     duration: Number,
  //     votes: Number,
  ...
  const tx = await writeNFT.setVoting(parentId, data);
  const hash = tx.hash;
  return getProvider()
    .waitForTransaction(hash)
    .then(() => {
...
const delVoting = async (parentId = []) => {
  const tx = await writeNFT.delVoting(parentId);
  const hash = tx.hash;
  return getProvider()
    .waitForTransaction(hash)
    .then(() => {
...
const stopVoting = async (parentId = [], duration = []) => {
  const tx = await writeNFT.stopVoting(parentId, duration);
  const hash = tx.hash;
  return getProvider()
    .waitForTransaction(hash)
    .then(() => {
...
const vote = async (parentId) => {
  const tx = await writeNFT.vote(parentId);
  const hash = tx.hash;
  return getProvider()
    .waitForTransaction(hash)
    .then(() => {
...
const protest = async (parentId) => {
  const tx = await writeNFT.protest(parentId);
  const hash = tx.hash;
  return getProvider()
    .waitForTransaction(hash)
    .then(() => {
